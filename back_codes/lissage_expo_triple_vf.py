...
def prediction_holtWinters(dataSet, p=12, sp=4, ahead=24,mtype='additive'):
    tsA = dataSet
    if mtype=='additive':
        results = holtWinters(tsA, p, sp, ahead, mtype = 'additive')
    else:
        results = holtWinters(tsA,p,sp,ahead,mtype = 'multiplicative')
    return results #results['smoothed'] est à afficher en plot
